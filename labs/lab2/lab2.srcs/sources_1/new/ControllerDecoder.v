...
module ControllerDecoder(
    ...
    begin
        ...
        /* FIXME: Write your code here... */
            ...
            op1_src = 1;    // op1取PC
            op2_src = 1;    // op2取立即数
            ALU_func = `ADD;    //ALU直接进行加法即可
            br_type = 0;    // 不是分支
            load_npc = 0;   // ALU结果写回寄存器
            wb_select = 0;  
            load_type = 0;
            reg_write_en = 1;   //写寄存器
            cache_write_en = 0;
            ...
            else if(funct3 == `B_BNE) begin
                br_type = `BNE;
            end
            else if(funct3 == `B_BLT) begin
                br_type = `BLT;
            end
            else if(funct3 == `B_BGE) begin
                br_type = `BGE;
            end
            else if(funct3 == `B_BLTU) begin
                br_type = `BLTU;
            end
            else if(funct3 == `B_BGEU) begin
                br_type = `BGEU;
            end
            else begin
                br_type = `NOBRANCH;
            ...
            else if(funct3 == `I_SLTI) begin
                ALU_func = `SLT;
            end
            else if(funct3 == `I_SLTIU) begin
                ALU_func = `SLTU;
            end
            else if(funct3 == `I_XORI) begin
                ALU_func = `XOR;
            end
            else if(funct3 == `I_ORI) begin
                ALU_func = `OR;
            end
            else if(funct3 == `I_ANDI) begin
                ALU_func = `AND;
            end
            else if(funct3 == `I_SLLI) begin
                ALU_func = `SLL;
            end
            else if(funct3 == `I_SR) begin
                if(funct7 == `I_SRLI) begin
                    ALU_func = `SRL;
                end
                else if(funct7 == `I_SRAI) begin
                    ALU_func = `SRA;
                end
                else begin
                    ALU_func = 0;
            ...
            else if(funct3 == `S_SH)    begin
                cache_write_en = 4'b0011;
            end
            else if(funct3 == `S_SW)    begin
            ...
            else if(funct3 == `R_SLL)   begin
                ALU_func = `SLL;
            end
            else if(funct3 == `R_SLT)   begin
                ALU_func = `SLT;
            end
            else if(funct3 == `R_SLTU)  begin
                ALU_func = `SLTU;
            end
            else if(funct3 == `R_XOR)   begin
                ALU_func = `XOR;
            end
            else if(funct3 == `R_OR)    begin
                ALU_func = `OR;
            end
            else if(funct3 == `R_AND)   begin
                ALU_func = `AND;
            end
            else if(funct3 == `R_SR)    begin
                if(funct7 == `R_SRL)    begin
                    ALU_func = `SRL;
                end
                else if(funct7 == `R_SRA)   begin
                    ALU_func = `SRA;
                end
                else    begin
                    ALU_func = 0;
        ...
        else if(opcode == `I_CSR)   begin   // TODO: CSR
            jal = 0;
            jalr = 0;
            op1_src = 0;
            op2_src = 0;
